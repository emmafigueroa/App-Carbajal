                        ...
                        <div class="input-student">
                            <div class="personal-information">
                                ...
                                <div class="birthday-date">
                                    ...
                                    <input type="date" name="birthday-date" id="birthday-date" value="<?php if (isset($_POST['birthday-date'])) {
...
valor de la variable de sesión a la variable . */
                    ...
                    $query_student = $mysqli->query("SELECT*FROM alumno WHERE id_alumno ='$id_student'");
                    if(isset($query_student)){
                        $fetch_student = $query_student->fetch_assoc();

                        if(isset($fetch_student['nombre_completo'])) $full_name = $fetch_student['nombre_completo'];
                        if(isset($fetch_student['genero'])) $gender = $fetch_student['genero'];
                        if(isset($fetch_student['grado'])) $grade = $fetch_student['grado'];
                        if(isset($fetch_student['grupo'])) $group = $fetch_student['grupo'];

                        $query_add_baja = $mysqli->query("INSERT INTO baja(nombre,genero,grado,grupo) VALUES('$full_name','$gender','$grade','$group')");
                        if(isset($query_add_baja)){
                            $query_delete_student = $mysqli->query("DELETE FROM alumno WHERE id_alumno ='$id_student'");
                            if (isset($query_delete_student)) {
                                header('Location: student.php');
                            }
                        }
                    }                    
                ...
                if (isset($_POST['search-student'])) {
                    $search_student = $mysqli->real_escape_string($_POST['search-student']);


                    if ($search_student == null) {
                        $query_search_student = $mysqli->query("SELECT*FROM alumno ORDER BY grado, grupo, nombre_completo");
                    } else {
                        if ($search_student == "F" || $search_student == "f" || $search_student == "M" || $search_student == "m") {
                            $query_search_student = $mysqli->query("SELECT*FROM alumno WHERE genero LIKE '%$search_student%' ORDER BY grado, grupo, nombre_completo");
                        } else if ($search_student == "1" || $search_student == "2" || $search_student == "3" || $search_student == "4" || $search_student == "5" || $search_student == "6") {
                            $query_search_student = $mysqli->query("SELECT*FROM alumno WHERE grado LIKE '%$search_student%' ORDER BY grupo, nombre_completo");
                        } else if ($search_student == "A" || $search_student == "a" || $search_student == "B" || $search_student == "b") {
                            $query_search_student = $mysqli->query("SELECT*FROM alumno WHERE grupo LIKE '%$search_student%' ORDER BY grado, nombre_completo");
                        } else {
                            $query_search_student = $mysqli->query("SELECT*FROM alumno WHERE nombre_completo LIKE '%$search_student%' ORDER BY nombre_completo");
                        }
                    }
                } else {
                    $query_search_student = $mysqli->query("SELECT*FROM alumno ORDER BY grado, grupo, nombre_completo");
            ...
            ?>
                <section class="show-student">
                    <form action="" method="post">
                        <input type="search" placeholder="Buscar" name="search-student">
                        <a href="add-student.php" class="add"><i class='bx bxs-plus-circle'></i></a>
                        <a href="qualification.php" class="assign"><i class="bx bxs-calculator"></i>Calificaciones</a>
                    </form>
                    ...
                    <table>
                        ...
                        <?php
                        while ($fetch_show_student = $query_search_student->fetch_assoc()) {
                        ?>
                            <tr>
                                <td><?php echo $fetch_show_student['nombre_completo']; ?></td>
                                <td><?php echo $fetch_show_student['grado']; ?></td>
                                <td><?php echo $fetch_show_student['grupo']; ?></td>
                                <td><?php echo $fetch_show_student['genero']; ?></td>
                                <td><?php echo $fetch_show_student['fecha_nacimiento']; ?></td>
                                <td><?php echo $fetch_show_student['edad']; ?></td>
                                <td>
                                    <div class="actions four">
                                        <form action="edit-student.php" method="post">
                                            <input type="hidden" name="id-student" value="<?php echo $fetch_show_student['id_alumno']; ?>">
                                            <button type="submit" name="edit-student"><i class='bx bxs-pencil'></i></button>
                                        </form>
                                        <form action="add-qualification.php" method="post">
                                            <input type="hidden" name="id-student" value="<?php echo $fetch_show_student['id_alumno']; ?>">
                                            <button type="submit" name="add-qualification"><i class='bx bxs-calculator'></i></button>
                                        </form>
                                        <form action="report-card.php" method="post">
                                            <input type="hidden" name="id-student" value="<?php echo $fetch_show_student['id_alumno']; ?>">
                                            <button type="submit" name="report-card"><i class='bx bxs-book-content'></i></button>
                                        </form>
                                        <form action="" method="post">
                                            <input type="hidden" name="id-student" value="<?php echo $fetch_show_student['id_alumno']; ?>">
                                            <button type="submit" name="delete-student"><i class='bx bxs-trash'></i></button>
                                        </form>
                                    </div>
                                </td>
